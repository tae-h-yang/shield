            ...
            for foe_id in foe_ids:
                if get_distance(ego_vehicle_id, foe_id) < 16:
                    if traci.vehicle.getAcceleration(foe_id) < -8:
                        print(traci.vehicle.getAcceleration(foe_id))
                        print("emergency braking")

            # print("Max decel: ", traci.vehicle.getDecel(ego_vehicle_id))
            # print("Max emer decel: ", traci.vehicle.getEmergencyDecel(ego_vehicle_id))
            # for neighbor in neighbors:
            #     foe_id = neighbor[0]
            #     decel = traci.vehicle.getAcceleration(foe_id)
            #     print("neighbor: ", foe_id)

            #     if decel > traci.vehicle.getDecel(foe_id):
            #         print("reward penelized.")
